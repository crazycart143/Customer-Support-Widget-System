<script lang="ts">
  ...
  import { MessageCircle, X, Send, Paperclip, ChevronLeft, Plus, History, Clock } from 'lucide-svelte';
  ...
  async function fetchMessages(ticketId: string) {
    ...
    // Subscribe
    supabase.channel(`customer_ticket_${ticketId}`)
      .on('postgres_changes', { 
        event: 'INSERT', 
        schema: 'public', 
        table: 'messages',
  ...
  async function sendMessage() {
    ...
    if (!error) {
      await supabase.from('tickets').update({ updated_at: new Date() }).eq('id', currentTicketId);
    }
  }
  ...
  async function handleFileUpload(event: Event) {
    ...
    try {
      ...
      const { data: msgData, error: msgError } = await supabase
        .from('messages')
        .insert([
          { ticket_id: currentTicketId, sender_type: 'customer', content: `Uploaded an attachment: ${file.name}` }
        ])
        .select()
        .single();
...
<div class="fixed bottom-6 right-6 z-[9999] font-sans">
    ...
    >
      ...
      <div class="bg-indigo-600 p-6 text-white shrink-0">
        <div class="flex justify-between items-start mb-4">
          <div>
            {#if view !== 'home'}
              <button on:click={() => view = 'home'} class="p-1 -ml-1 hover:bg-white/20 rounded-lg transition-colors mb-2">
          ...
          <button on:click={() => isOpen = false} class="p-2 hover:bg-white/20 rounded-full transition-colors">
            <X class="w-5 h-5" />
          </button>
        </div>
        {#if view === 'home'}
          <p class="text-indigo-100 text-sm">We're here to help you with any questions or issues you might have.</p>
        {/if}
      </div>
      ...
      <div class="flex-1 overflow-y-auto bg-slate-50 relative">
        {#if view === 'home'}
          <div class="p-6 space-y-4" transition:fade>
            ...
            >
              <div class="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 group-hover:scale-110 transition-transform">
                <Plus class="w-6 h-6" />
              </div>
              <div>
                <h3 class="font-bold text-slate-900">Start new conversation</h3>
                <p class="text-xs text-slate-500 mt-0.5">We'll get back to you shortly</p>
              </div>
            </button>

            <button 
              on:click={() => { view = 'history'; fetchHistory(); }}
              class="w-full bg-white p-6 rounded-2xl shadow-sm border border-slate-100 text-left flex items-center gap-4 hover:border-indigo-200 hover:bg-indigo-50/50 transition-all group"
            >
              <div class="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-600 group-hover:scale-110 transition-transform">
                <History class="w-6 h-6" />
              </div>
              <div>
                <h3 class="font-bold text-slate-900">See previous tickets</h3>
                <p class="text-xs text-slate-500 mt-0.5">View your conversation history</p>
              </div>
            </button>
          </div>
        ...
        {:else if view === 'new-ticket'}
          <div class="p-6 space-y-6" transition:fade>
            <div class="space-y-2">
              <label for="subject" class="text-sm font-semibold text-slate-700">What do you need help with?</label>
              <input 
                id="subject"
                type="text" 
                bind:value={subject}
            ...
            {#each tickets as t}
              ...
              >
                <div class="flex justify-between items-start mb-2">
                  <h4 class="font-bold text-slate-900 text-sm line-clamp-1">{t.subject}</h4>
                  <span class={clsx("w-2 h-2 rounded-full mt-1.5", statusColors[t.status])}></span>
                </div>
                <div class="flex items-center gap-2 text-[10px] text-slate-400">
                  <Clock class="w-3 h-3" />
                  <span>{new Date(t.updated_at).toLocaleDateString()}</span>
                  <span class="uppercase font-bold tracking-tighter text-slate-300 ml-auto">{t.status}</span>
                </div>
              </button>
            {/each}
          </div>
        ...
        {:else if view === 'chat'}
          <div id="widget-chat-messages" class="h-full overflow-y-auto p-4 space-y-4 pb-20">
            {#each messages as msg}
              <div class={clsx("flex flex-col max-w-[85%]", msg.sender_type === 'customer' ? "ml-auto items-end" : "mr-auto items-start")}>
                <div class={clsx(
          ...
          <div class="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100">
            <form on:submit|preventDefault={sendMessage} class="flex items-center gap-2">
              ...
              >
                <Paperclip class={clsx("w-5 h-5", uploadingFile && "animate-pulse")} />
              </button>
              <input 
                type="text" 
  ...
  >
    ...
    {/if}
    <div class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full border-2 border-white scale-0 group-hover:scale-100 transition-transform"></div>
  </button>
</div>
