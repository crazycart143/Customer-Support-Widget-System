...
<div class="h-[calc(100vh-140px)] flex flex-col gap-4">
  ...
  <div
    class="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden"
  >
          ...
          >
            <div class="flex items-center gap-2 mb-1">
              {#if msg.sender_type === "admin"}
                ...
                <span
                  class="text-[10px] font-bold text-indigo-600 uppercase tracking-tight"
                  >Support Agent</span
                >
              {:else}
                ...
                <div class="mt-3 space-y-2">
                  {#each msg.attachments as att}
                    {#if att.content_type?.startsWith("image/")}
                      <div
                        class="rounded-lg overflow-hidden border border-black/5 max-w-sm"
                      >
                        <img
    ...
    <div class="p-4 bg-slate-50 border-t border-slate-200">
      {#if ticket?.status === "resolved" || ticket?.status === "closed"}
        <div
          class="text-center py-4 bg-white border border-slate-200 rounded-xl"
        >
          <p class="text-sm font-bold uppercase tracking-widest text-slate-400">
            This ticket is {ticket?.status} and cannot be replied to.
          </p>
        </div>
      {:else}
